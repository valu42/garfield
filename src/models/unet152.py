...
class UNetResNet152(nn.Module):
    ...
    def __init__(
        ...
        # Modify the first convolutional layer to accept more input channels
        if in_channels != 3:
            # Create a new conv1 with the specified number of input channels
            # but keep the same output channels, kernel size, stride, etc.
...
def build_unet(
    ...
    Args:
        ...
        decoder_channels: List of decoder channel counts
        use_batchnorm: Whether to use batch normalization
        in_channels: Number of input channels (3 for RGB, 6 for RGB+FFT)
    ...
    model = UNetResNet152(
        ...
        use_batchnorm=use_batchnorm,
        in_channels=in_channels,  # Pass this parameter
    )
