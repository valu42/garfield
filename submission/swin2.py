...
import timm
from timm.models.swin_transformer_v2 import SwinTransformerV2  # Import Swin V2
...
class SwinEncoderWrapper(nn.Module):
    """
    A simplified encoder that runs the input sequentially through the Swin Transformer V2's patch
    embedding and layers. It converts the patch embedding output into [B, H, W, C] format,
    then converts intermediate features to [B, C, H, W] for the decoder.
    """
    def __init__(
        self,
        img_size: int = 256,   # Using 256 to match your data.
        patch_size: int = 4,
        in_chans: int = 3,
        embed_dim: int = 192,
        depths: List[int] = [2, 2, 18, 2],
        num_heads: List[int] = [6, 12, 24, 48],
        window_size: int = 12,  # Window size will be internally managed.
        mlp_ratio: float = 4.0,
        qkv_bias: bool = True,
        drop_rate: float = 0.0,
        ...
        if pretrained is True:
            # Use timm's built-in function with the largest pretrained model for 256x256 inputs.
            ...
            print("Using pretrained SwinV2 Large (window12to16) model for 256x256!")
        else:
...
class DirectSwinUNet(nn.Module):
    """
    A UNet-like segmentation model using the simplified Swin Transformer V2 encoder.
...
if __name__ == "__main__":
    # Build model with img_size=256, which is suitable for your data.
    model = build_swin_large_unet(
        ...
        num_classes=1,
        window_size=12,
        pretrained=True,  # This will use 'swinv2_large_window12to16_192to256.ms_in22k_ft_in1k'
    )
